...
import { generateGearsDelivery } from '../src/tools/generate-gears-delivery.js';

const tmpDir = path.join(os.tmpdir(), 'kb-gears-delivery-test-' + Date.now());
const dataRoot = path.join(tmpDir, 'data');
const projectId = '20260617-story-gears--ai_comic_drama';
const storyId = '20260617-story-gears';
...
describe('kb_generate_gears_delivery', () => {
  it('generates a read-only GEARS delivery package from project_id', async () => {
    const result = await generateGearsDelivery({ project_id: projectId, include_markdown: false });
    ...
    expect(result!.package.scene_assets[0].description).not.toContain('质量信号');
    expect(result!.package.validation_notes.some(note => note.includes('visual/camera prompt'))).toBe(true);
    expect(result!.package.validation_notes.some(note => note.includes('segment_prompt_hint'))).toBe(true);
  ...
  it('can resolve a story by story_id', async () => {
    const result = await generateGearsDelivery({ story_id: storyId, include_markdown: false });
  ...
  it('returns null for missing projects and rejects unsafe ids', async () => {
    await expect(generateGearsDelivery({ project_id: '../bad' })).rejects.toThrow('非法项目 ID');
    await expect(generateGearsDelivery({ project_id: 'missing-project' })).resolves.toBeNull();
  });
});
