...
4. 把 readiness 接进质量报告和 GEARS 交付状态：核心链路、前端显示、Seedance 本地参考资产占位、GEARS 本地验收边界、外部回片交接队列、系统级批量 preflight/import、worker acceptance runbook、import 回执 URL 复核和 verdict 前置门禁已完成；下一步做真实 GEARS/Seedance worker 实跑、状态同步和证据签收。
