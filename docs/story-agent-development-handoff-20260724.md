...
| Reference Library UI | P1-A2c 治理工作台已完成三轮 | 已有来源元数据录入、来源选择、pending film/text analysis、`material:sign` 独立批准、任务/evidence 账本，以及跨来源 approved analysis → benchmark → style pack 组合；仍缺 baseline 对照 UI |
...
P0-A、P0-B、P0-C、P0-D、P0-E1、P0-E2、P1-A1、P1-A2a、P1-A2b、P1-A2c task contract、来源/analysis 治理、task/evidence 与 benchmark/style-pack 组合 Web 工作台已完成。不要再调查视频、后期或真人测试，也不要重复开发已经证明的本地恢复、record-replay、strict fallback、approved style-pack prompt/trace 桥接、合法 evidence、多维相似度、baseline、analysis-task 合同或已完成的 Reference Library 页面。
...
若没有凭据，不要伪造实跑，直接进入 P1-A2c 后续：建设 Reference Library baseline 对照 UI，或进入更高层统一 StoryAgentRun。只有用户提供合法资料并亲自确认授权声明后，才通过现有 manifest 运行 operator-submitted evidence 的真实四维门禁。没有真实外部 Provider 时不要把 fixture baseline 写成真实对照。
