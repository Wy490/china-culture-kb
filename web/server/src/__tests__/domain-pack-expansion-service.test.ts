...
describe('domain-pack-expansion-service', () => {
  it('reports review-gated Domain Pack expansion candidate batches', () => {
    ...
    expect(report.review_packet.review_status_counts?.approved).toBe(50);
    expect(report.review_packet.approved_writeback_draft_count).toBe(50);
    expect(report.writeback_preflight).toMatchObject({
      ...
      manual_review_required_count: 50,
    ...
    expect(report.markdown).toContain('progress_percent: 93');
    ...
    expect(report.markdown).toContain('approved_draft_count: 50');
  ...
  it('loads tracked review seeds and lets runtime review state override them', () => {
    ...
    expect(report.review_packet.approved_writeback_draft_count).toBe(49);
  ...
  it('bulk updates filtered review candidates without writing province markdown', () => {
    ...
    expect(update.result?.report.review_packet.batches
      .flatMap(batch => batch.review_items)
      .filter(item => item.review_status === 'approved')).toHaveLength(50);
    ...
    expect(draftPackage.items.filter(item => item.writeback_status === 'draft_ready')).toHaveLength(48);
