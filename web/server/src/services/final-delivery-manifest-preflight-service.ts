...
import type {
  StoryAgentFinalDeliveryManifestPreflightCheck,
...
async function inspectGeneratedPath(
  ...
  if (isAbsolute(declaredPath) || /^[a-z][a-z0-9+.-]*:\/\//i.test(declaredPath)) {
    return { label, declared_path: declaredPath, safe: false, exists: false, reason: 'absolute_or_url_path' };
  }
  const resolvedPath = resolve(generatedRoot, declaredPath);
  const relation = relative(generatedRoot, resolvedPath);
  const insideGeneratedRoot = relation !== ''
    && relation !== '..'
    && !relation.startsWith(`..${sep}`)
    && !isAbsolute(relation);
  const projectScoped = declaredPath.split(/[\\/]+/).includes(projectId);
  if (!insideGeneratedRoot || !projectScoped) {
    return {
      label,
      declared_path: declaredPath,
      safe: false,
      exists: false,
      reason: insideGeneratedRoot ? 'path_not_project_scoped' : 'path_escapes_generated_root',
    };
  }
  return {
    label,
...
function notApplicableCheck(
  key: StoryAgentFinalDeliveryManifestPreflightCheck['key'],
  reason: string,
): StoryAgentFinalDeliveryManifestPreflightCheck {
...
export async function preflightStoryAgentFinalDeliveryManifest(
  ...
  const safeProjectId = projectIdIsSafe(projectId)
    && relative(projectRoot, projectDir) === projectId;
  let projectRecord: JsonRecord | undefined;
  if (safeProjectId) {
    try {
      projectRecord = asRecord(JSON.parse(await readFile(resolve(projectDir, 'project.json'), 'utf8')));
    } catch {
  ...
  const checks: StoryAgentFinalDeliveryManifestPreflightCheck[] = [
    {
      key: 'target_exists',
  ...
  if (input.disposition === 'preserve_fixture_exclude_from_publishable_delivery') {
    ...
    for (const spec of dependencySpecs) {
      const inspected = await Promise.all(spec.paths.map(([label, declaredPath]) =>
        inspectGeneratedPath(generatedRoot, projectId, label, declaredPath)));
    ...
    const finalOutputPath = asString(finalDelivery.output_path);
    ...
    const inspectedProjectPaths = await Promise.all([
      ...inspectedDependencyPaths.map(item => inspectGeneratedPath(
        generatedRoot,
        projectId,
        item.label,
        item.declared_path,
      )),
      inspectGeneratedPath(generatedRoot, projectId, 'final_delivery_output', finalOutputPath),
      inspectGeneratedPath(generatedRoot, projectId, 'prospective_manifest', prospectiveManifestPath),
    ]);
