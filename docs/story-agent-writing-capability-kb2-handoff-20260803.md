...
- 前序 B4 与 M4 第一至第十九切片已于 `15fb8681` 提交并推送；当前工作区包含尚未提交的 M4 第二十至第二十四切片（历史剧情、传说故事、文化宣传、纪录短片与山水意境生成/派生治理）
...
M2 剩余 1% 是机器报告固化，不是产品主链缺失。M3 原始字段治理已完成；M4 故事可发布、GEARS 合同与创作质量均已 45/45，机器创作质量不变量整体通过。生成后生产素材重算已把 production material 缺口从 42 降到 13、开放动作从 86 降到 25；历史剧情、传说故事和山水意境三变体均已通过生产素材门禁。文化宣传只剩真实权利/署名，纪录短片只剩真实采访选段。下一重点是城市品牌混合缺口与人物改编压力例，而不是放宽门禁。
...
2. 继续强化已接入的跨条目 Domain Pack：
   ...
   - 语言语体与地域表达（已完成首包）；
   - 自然环境、季节、天气和声景（已完成首包）。
3. 跨包检索、readiness、quality、repair、片型预期包映射、生成前反事实对照与生成后 readiness 重算已完成；原始字段 B1—B4 已全部清零；15 类型 × 3 样本的稳定性、诊断聚类、幂等和创作/生产分层指标已固化，故事可发布、GEARS 与创作质量均已 45/45。场景短片、历史剧情、传说故事、文化宣传、纪录短片与山水意境的可生成结构已收口。下一步推进剩余 13 项 production material 缺口和 25 个开放动作，优先城市品牌混合缺口与人物改编压力例。不得把 prompt 差异或专业候选机器分转述为成片质量提升。
...
2. 当前指标已明确拆分：legacy 综合 32/45、创作质量 45/45、故事可发布 45/45、生产素材 32/45、全生产就绪 0/45。场景短片、历史剧情、传说故事和山水意境三变体已全部通过生产素材门禁；文化宣传只保留真实权利门禁，纪录短片只保留真实采访选段，M4 机器创作质量不变量整体 `passed`。下一切片处理 13 个 production material 缺口和 25 个开放动作。仍须保留 weak pattern 作为质量优化线索，不得把机器 `passed` 冒充真人、专业、外部模型或生产信用。
3. 修复后继续比较事实/文化边界、类型完成度、结构、场景可拍性、实际 repair trace 和稳定性，保持同一 45 案例口径。
4. 按当前用户指令，人工盲评不作为工程启动前置，但机器报告不得冒充真人反馈。

#### 已入库的剩余开发 backlog（v29）

1. 可由生成器/派生结构继续治理：`character_story` 改编压力例的 `life_stage_window` 1 例。
2. 混合缺口：`city_brand_promo` 的城市身份、受众、声景、游客行动、路线与天气预案可结构化派生，但场地许可必须真实提供。
3. 只认真实源素材：`documentary_short` 的采访选段 3 例、`culture_promo` 的权利与署名 3 例、`heritage_promo` 的官方目录/资源链接 3 例，以及授权、场地许可、参考图和采访同意；不得从生成正文中伪派生。
...
前序 B4 与 M4 第一至第十九切片已在 `15fb8681` 提交并推送；当前第二十至第二十四切片尚未提交。先核对分支、HEAD、git status、staged 状态和 M4 机器基线，不要假设交接数字仍然有效，也不要重复实现已完成能力。

当前优先级是把功能做全、把能力做好；人工评审、真人流程和用户注册不作为工程前置，但不得虚构人工信用。生成故事和机器派生生产指导不得写回 data/provinces/*.md。

全国基础覆盖机器目标已经完成：289 条、1043 个来源、34/34 地区至少 5 条。Domain Pack 与生成后 readiness 主链已完成，原始字段 B1—B4 已全部清零。M4 15×3 v29 当前正式基线：legacy 综合 32/45、创作质量 45/45、故事可发布 45/45、生产素材 32/45、全生产就绪 0/45；repair attempted 0、applied 0、开放动作 25，GEARS 45/45。production material 缺口已由 42 降到 13；场景短片、历史剧情、传说故事和山水意境三变体均通过生产素材门禁，文化宣传只剩真实权利/署名，纪录短片只剩真实采访选段。官方链接、真实授权、参考图、场地许可等仍只认源素材；45/45 仍受真实资产与外部 Provider gate 阻断。下一切片优先城市品牌混合缺口与人物改编压力例。
...
- B4 与 M4 第一至第十九切片已在 `15fb8681` 提交并推送；M4 第二十至第二十四切片代码、v29 报告和交接更新当前尚未提交，精确状态以 `git status -sb` 为准。
