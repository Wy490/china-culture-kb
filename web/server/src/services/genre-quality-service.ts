...
function hasSemanticSignalEvidence(text: string, signal: string): boolean {
  ...
  const checks: Array<[RegExp, RegExp[]]> = [
    ...
    [/阻力具体|必须有阻力|制度压力可见/, [/官场规则/, /制度压力/, /时代压力/, /军阀统治/, /社会动荡/, /地方权势/, /谷价/, /租息/, /名声/, /人情/, /催客/, /浊浪/, /路远/, /书卷会湿/, /行程.{0,6}误/, /知军.{0,8}催/, /催他签字/, /此案已定/, /得罪上官/, /可能丢官/, /获罪/, /长官权威/]],
    ...
    [/因果链清楚|事件因果清楚/, [/因为/, /因此/, /于是/, /导致/, /若[^。；]+；若/, /才/, /看见.{0,12}生出/, /生出.{0,12}承担/, /愿意承担.{0,12}才/, /忽然发现/, /发现疑点/, /疑点重重/, /证词前后不合/, /证据不足/, /只待.{0,6}画押/]],
    [/人物不是年表|不得写成年表式介绍/, [/(少年周敦颐|周敦颐).*(背起|停下脚步|蹲下|挽起|踩进|写下|停住笔|重查|翻到案卷|不能签字|退回|交还任命文书|逐页细读|记录疑点)/, /(屈原).*(站在风里|走向汨罗江|整理衣冠|怀石)/, /(毛泽东).*(收起|离开|走进|围坐|放下|翻开|整理|徒步|组织|倾听|考察)/]],
    [/史实边界明确|再现边界清楚/, [/影视化创作/, /事实边界/, /史实边界/, /再现边界/, /确证/, /可考/, /不是《爱莲说》/, /不把.{0,20}写成/, /仍要说清/, /只作.{0,8}伏笔/, /据《史记》/, /传统叙述/, /有限再现/, /不把再现画面当作原始记录/, /事实梳理/]],
    [/制度压力可见|时代压力可见/, [/郢都失守/, /楚国败局/, /国都失陷/, /流放无归/, /军阀统治/, /社会动荡/, /时代压力/, /地方权势/, /谷价/, /租息/, /制度压力/, /上官/, /催签/, /权势/, /官场规则/]],
...
function storyText(story: StoryGenerateResult): string {
  return [
    ...
    ...(story.protagonist_arc ?? []).flatMap(arc => [
      arc.starting_state,
      arc.turning_point,
      arc.resolution,
    ]),
