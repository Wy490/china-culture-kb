...
- B4 与 M4 第一至第四十八切片均已完成；第四十八切片实现提交为 `0cf9843a`
...
第四十八切片已建立可复现的 RAG 检索评测基线，固定 16 个机器编制案例、语料指纹、Recall@K、MRR、NDCG、禁止条目与不可回答误报。当前 Recall@3/5 为 1、MRR@10 为 1、NDCG@5 为 0.977609，不可回答候选泄漏为 0；但可回答案例中只有 1/12 出现 `usable_for_story=true` 的相关条目，说明旧 0.75 接纳阈值与语义长查询的评分尺度明显不协调。下一主线应优先校准候选排序与可用性判定，同时继续准备结构化证据入链 shadow，不是无边界扩条。

### 4.1 RAG 三层完成度快照

以工程成熟度而非自动 SLA 计算，当前评估为：代码层 90%、检索层 80%、生成层 90%，综合约 86%。
...
- 检索层 80%：已有字段加权词法检索、意图扩展、长字段命中、多需求匹配、地域/片型/本地化约束、Domain Pack 增强，以及绑定数据集/语料指纹的 Recall/MRR/NDCG 与离域误报基线。尚无 embedding/混合召回/重排、大语料索引与缓存、部分读取失败完整性诊断；`usable_for_story` 可用级相关覆盖仅 8.33%，仍是明确的排序/校准缺口。
...
#### 已入库的剩余开发 backlog（第四十八切片后；15×3 正式快照为 v34）

1. 当前 15×3 矩阵已经没有可由生成器或派生结构继续解除的 production material 缺口；`character_story` 改编压力例的 `life_stage_window` 已由完整知识证据行严格派生。
2. RAG 检索与证据链：首个 16 案例基线已完成，下一步先增加主体意图、地域歧义和同名负例，然后优化主体级排序与 `usable_for_story` 校准；只有词法基线无法继续提升时再引入 embedding/混合召回/重排。同时以 shadow 对照证明 `story-knowledge-contract/v1` 进入生成链后不放大未分级来源，未达门禁前保持 `consumed_by_generation=false`。
...
B4 与 M4 第一至第四十八切片均已完成；第四十八切片实现提交为 `0cf9843a`。先核对分支、HEAD、git status、staged 状态和 M4 机器基线，确认交接文档提交与上游状态；不要假设交接数字仍然有效，也不要重复实现已完成能力。

当前优先级是把功能做全、把能力做好；人工评审、真人流程和用户注册不作为工程前置，但不得虚构人工信用。生成故事和机器派生生产指导不得写回 data/provinces/*.md。

全国基础覆盖机器目标已经完成：289 条、1043 个来源、34/34 地区至少 5 条。Domain Pack 与生成后 readiness 主链已完成，原始字段 B1—B4 已全部清零。M4 15×3 v34 当前正式快照：legacy 综合 33/45、创作质量 45/45、故事可发布 45/45、生产素材 33/45、全生产就绪 0/45；repair attempted 0、applied 0、开放动作 12，GEARS 45/45，平均 pattern 95.69，pattern 弱信号 0。RAG 证据分层与首个确定性检索基线均已完成：16 案例的 Recall@3/5=1、MRR@10=1、NDCG@5=0.977609，离域候选泄漏为 0；但可用级相关覆盖只有 1/12。下一优先级是主体排序与 `usable_for_story` 校准，以及结构化证据入链 shadow；不得简单放宽事实门禁或以生成内容伪造剩余 12 项。
...
- B4 与 M4 第一至第四十八切片均已完成；第四十八切片实现提交为 `0cf9843a`，交接文档提交与上游状态以最新 `git log` 与 `git status -sb` 为准。v34 正式快照的 pattern 开放目标与弱信号均为 0；RAG 当前工程成熟度评估为代码层 90%、检索层 80%、生成层 90%、综合约 86%。第一份语料绑定检索基线已入库，下一切片优先改善主体排序/`usable_for_story` 校准或结构化证据入链 shadow。
