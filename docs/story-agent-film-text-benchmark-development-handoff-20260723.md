...
这些事项被延后，不代表删除。隔离的回调与后期全生命周期测试已在第 15 节完成；当前下一步聚焦用更多系列种子重复 15 类型、图片资产和镜头链路，建立跨项目稳定性证据。

## 15. 2026-07-23 隔离回调与后期全生命周期烟测

新增可重复运行命令：

```bash
cd web
npm run smoke:story-agent-full-function
```

脚本 `web/server/scripts/story-agent-full-function-smoke.mts` 每次创建独立临时工作区，只读复用正式知识库数据，将所有生成项目、账本和合成制品写入临时目录，结束后删除。它不会读取或修改《皮影诡戏：守灯人》的正式系列账本，也不调用 GEARS、Seedance 或其他外部视频 Provider。
...
- 27 条字幕导出并完成烧录合同；
- 1 个本地音频资产绑定并完成混音合同；
- 8 张片头、分集标题和片尾卡完成渲染合同；
- 最终交付进入 ready；
- 交付 manifest 包含 3 个 deliverable；
- 当前 release 为 immutable，并生成 output / manifest 双 SHA-256。

这里的剪辑、字幕烧录、音频、标题卡和最终视频使用的是**合成文件字节**，目的是验证文件仓储、临时写入、原子发布、账本状态、依赖门禁、回调、重试、manifest 和不可变发布链。它们不是可播放视频，也不冒充真实 Provider 成片。

### 15.2 稳定性与自动化证据

同一命令连续运行四次均通过，且每次都删除隔离工作区：

```text
隔离全功能烟测：连续 4 次 passed
每次：3/3 集、27/27 镜头、1 次失败、1 次重试、0 回调导入失败
后期：cut ready、subtitle ready、audio ready、8/8 title cards ready、final ready
边界：0 外部调用、0 外部授权、0 真人审核要求、0 正式项目污染
系列/后期聚焦回归：1 file / 37 tests passed
服务端源码与 scripts 严格 TypeScript 检查：通过
```

脚本已由 `web/server/tsconfig.scripts.json` 纳入服务端 lint，避免烟测入口只有运行时保障而没有编译保障。结构化证据已合并到 `data/reports/story-agent-full-function-smoke-20260723.json` 的 `isolated_postproduction_lifecycle_smoke`。

### 15.3 当前结论与下一边界

原第 13、14 节所列“回调、制品导入、失败重试、剪辑、声音、字幕、片头和最终装配”已不再是待办：它们已有两轮隔离机器全链路证据。当前尚未完成的是两类不同性质的工作：

1. 用更多不同题材、集数和资产规模的系列种子重复全链路，证明跨项目稳定性；
2. 在未来获得凭证、预算和明确外呼授权后，用真实视频 Provider 替换 mocked 镜头回调，验证真实媒体质量与网络侧可靠性。

真人测试仍然不是当前研发门禁；production credit 仍保持严格语义，不会因为机器全链路通过而自动增加。

## 16. 2026-07-23 跨题材、跨节奏稳定性矩阵

新增可重复运行命令：

```bash
cd web
npm run smoke:story-agent-cross-seed
```

该命令复用第 15 节的隔离全生命周期机制，但不再只测单一系列。每次运行四个不同题材、不同集数、不同节奏的独立系列：

| 种子 | 题材 | 节奏 | 事实模式 | 集数 | 镜头 |
|---|---|---|---|---:|---:|
| 《潮汐失忆局》 | 近未来原创悬疑 | `mystery_cliffhanger` | `fictional_original` | 3 | 27 |
| 《告身不署》 | 历史伦理 | `slow_burn` | `source_adaptation` | 4 | 36 |
| 《一线醒狮》 | 非遗工艺 | `balanced_drama` | `source_adaptation` | 5 | 45 |
| 《白鹿送药记》 | 儿童传说 | `fast_hook` | `source_adaptation` | 2 | 18 |

每个种子都完整经过：系列规划、全部分集生成、分镜与 Seedance 提示词导出、本地 mocked GEARS 提交、主动注入一次失败、精确重试、全量回调、剪辑、字幕烧录、音频绑定与混音、标题卡、最终 manifest、不可变 release 和临时工作区清理。

### 16.1 质量门禁已升级为硬失败

跨种子矩阵不仅检查任务和文件状态，还将以下九项设为硬门禁；任一项失败，命令立即以非零状态结束：
...
5. 所有商业机器门禁通过；
6. 所有镜头进入 ready；
7. 所有主动注入的失败都被精确重试并恢复；
8. 剪辑、字幕、音频、标题卡和最终交付全部 ready；
9. 所有最终 release 均为 immutable。

这修正了此前“交付链成功，但内容质量失败仍显示 passed”的缺口。

### 16.2 跨种子暴露并修复的缺陷

第一轮矩阵真实暴露了五类跨题材稳定性问题：

1. 文化知识已经完整覆盖必需项、但条目位于 supporting role 时，被错误判为 `fictional_original`；
2. “记忆修理师顾弦”“青年绣娘苏翎”“少年药童小禾”等职业称谓中的姓名未进入前提合同；
3. 人物状态检测把“失踪乘客”和普通“行动”误判为角色离场后复活；
4. “传说/待核内容不得写成确证史实”的明确谨慎边界，被误判为待核与确证冲突；
5. 周敦颐拒签题材的分集伏笔没有携带计划线索标题，被错误报告为孤立伏笔。

修复后，两个连续完整运行均得到同一功能结论：

```text
跨种子稳定性矩阵：连续 2 次 passed
覆盖：4 seeds / 4 pacing profiles / 2 truth modes
每次：14/14 集、126/126 镜头、4 次失败注入、4 次精确重试、36 张标题卡、4 个 immutable release
质量：4/4 系列质量 100；4/4 前提忠实度 100；4/4 商业机器门禁通过
连续性：0 blocking memory conflict；0 thread-closure issue
边界：0 外部调用、0 外部授权、0 真人审核要求、0 正式项目污染
```
...
全仓 check：visible copy 9 files / 17 checks、server source/scripts tsc、client vue-tsc 全部通过
生产构建：server tsup、client Vite 通过；171 modules transformed
跨种子矩阵：连续 2 次全硬门禁通过
```
