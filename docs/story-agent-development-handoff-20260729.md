...
### 6.8 本轮完成：非遗、纪录与讲解 canonical 配方扩展

canonical 配方目录从 8 个扩展到 11 个，成片类型覆盖从 5/15 提升到 8/15：

- `heritage_craft_process_evidence` 面向非遗/工艺宣传片，以原料状态、工具、关键工序、
  失败风险和实践者传承关系建立过程证据；
- `documentary_evidence_trail` 面向微纪录片，从现实现场提出问题，分层组织实物、
  文献和受约束解释，并明确已证实、合理推断与待核验内容；
- `explainer_question_to_example` 面向知识讲解视频，以核心问题、概念、实例、反例和
  回扣形成可复述的解释链；
- 三个配方均使用现有类型专属 `GenreStoryProfile` 和 narrative pattern 兼容关系，
  未在 prompt、fallback 或 UI 中旁路新增类型规则；
- 每个配方都具有稳定 canonical SHA-256，服务端继续逐字段复验，篡改 fail closed；
- 推荐器新增确定性素材特征和关键词信号，事实型/机构任务仍优先执行真实性与材料
  充足度边界；
- Story Studio 新增“知识与纪录机制”分类标签；
...
```text
TDD red
  5 expected failures observed
  existing 7 related tests remained green
...
Raw generated audit
  1284 total targets
  321 ready / 2 planned / 35 production_gap / 926 interrupted

Active signoff portfolio
  457 targets
  321 ready / 2 planned / 35 production_gap / 99 interrupted
  827 historical series reversibly excluded by soft-archive manifest

Behavior
  raw totals remain available for audit
  excluded historical fixtures do not enter the active MVP denominator or blockers
  99 active interrupted series still keep generated_artifacts blocked
  no project JSON was deleted, rewritten, or credited as ready
...
Server TypeScript lint / MCP TypeScript build
  passed
```
...
ed8f0efd feat(story-agent): add human recipe review ledger
