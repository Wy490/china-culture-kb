...
- 第五十三切片实现提交：`cf649fcf`（`feat(story-agent): bind prompt canary decisions`）
- 第五十四切片实现提交：`779cb324`（`perf(story-agent): share request project projection`）
...
- B4 与 M4 第一至第五十六切片均已完成；第五十六切片实现提交为 `39b79f6b`
...
以工程成熟度而非自动 SLA 计算，当前评估为：代码层 94%、检索层 85%、生成层 94%，综合约 91%。

- 代码层 94%：已有独立类型/schema、Domain Pack、REST/MCP、KnowledgePack/MaterialPack/Blueprint、质量/repair、项目版本持久化和大量回归门禁；结构化证据合同已进入生成准备与 prompt-package shadow，并有全库审计、approved overlay 正负矩阵、受限只读 canary、制品绑定验证器和多案例迁移决策基线。剩余主要是结构化证据尚未成为生成链单一事实源、多条 parser/打分路径的一致性成本，以及真实审批/生产证据依赖。
...
- 生成层 94%：知识上下文已进入蓝图、外部 prompt、本地 fallback、场景/GEARS、质量与 repair，v34 机器样本保持故事可发布 45/45、GEARS 45/45；`story-knowledge-contract/v1` 已进入生成准备和未执行 prompt-package 的只读 fail-closed shadow，7/7 对照证明正式 prompt 与故事输出不变，受限 canary 可输出并验证不可消费的绑定迁移决策。剩余核心缺口是正式消费所需的真实核验/独立审批/生产 canary 证据，以及真实外部模型、真人/专业评审与成片生产信用。
...
- `data/reports/story-agent-story-knowledge-prompt-shadow-canary-decision-v1.json`（7 路只读 canary receipt 绑定与不可消费迁移决策基线）
...
- canary receipt 新增五段规范化 SHA-256 绑定：解析后的 generation request、可选 evidence overlay、generation shadow、prompt shadow comparison 和 migration decision。migration decision 自身再绑定 generation shadow 与 prompt comparison，避免决策脱离所评估制品被替换。
- 新增 `verifyStoryKnowledgePromptShadowCanaryReceipt`：同时解析 request/receipt schema 并重算所有绑定；篡改 `changed_prompt_package_paths` 的红测会明确得到 `prompt_shadow_comparison_sha256_mismatch` 与 `migration_decision_prompt_shadow_binding_mismatch`，不再只检查哈希字符串格式。
- prompt-shadow matrix 的 7 路 fixture 定义抽为共享确定性构建器，原矩阵仍保持 1 candidate-ready、2 blocked、4 safe-no，7/7 active prompt 与故事结果一致、shadow 执行 0；共享 fixture 不改变正式 execution。
- 新增 `story-knowledge-prompt-shadow-canary-decision-audit/v1` 机器报告和 `audit:story-knowledge-prompt-shadow-canary-decision` 的 `--write`/`--check` 脚本。审计覆盖 base-only、approved multi-source、machine-only、mixed certainty、pending、rejected、incompatible 共 7 路：1 路仅达到 `eligible_for_operator_review`，6 路 `remain_shadow`；7/7 receipt 绑定有效，正式消费、adapter 调用和持久化均为 0。
- canary/审计/原矩阵/preparation 定向 4 文件/19 项通过；新报告 `--write` 后逐字节 `--check` 通过，原 prompt-shadow matrix 也逐字节不漂移。Server lint/build 与 `git diff --check` 通过；沙箱外最终 Server 全量 219 文件/1816 项通过、1 文件/2 项既有跳过、0 失败。实现提交为 `cf649fcf`。
- 所有 overlay/reviewer 仍是合成 fixture；报告固定 `human_review_complete=false`、`production_canary_executed=false`，不写 `data/provinces/*.md`，不授予真人、事实核验、外部模型、生产流量或成片信用。知识 shadow 的下一工程步骤已受真实证据约束；后续优先转向大项目库共享只读请求投影，或等待真实核验/审批/生产 canary 证据，不创建虚假的正式消费开关。

M4 第五十四切片完成时：

- 新增 `ProjectReadOnlyRequestProjection` 请求级只读上下文：同一 MVP 聚合请求内惰性去重 `listProjectIds`，并按项目缓存 `inspectCurrentStateReadOnly` 的成功、结构不可读和读取异常结果。返回边界固定为 request-scoped、read-only、无仓储写权限、项目失败隔离、禁止跨请求缓存。
- Story Agent MVP 的 supplement task snapshot 与 external evidence sync-health 现共享一个投影；前者直接从经验证的 current snapshot 规范化故事并派生补录任务，后者复用相同 current-state 或失败分类。普通补录任务 API 未注入投影时继续走既有 `listProjects/getProject` 路径，production portfolio 也继续保留历史版本与系列项目合同，没有为减少扫描而改变语义。
- `story-agent-mvp-status/v1.performance` 新增项目 ID 列表仓储/缓存命中、current-state 仓储/缓存命中、可读/失败项目数六项诊断。聚合 API 回归强制项目 ID 仓储调用为 1，current-state 第二次消费者缓存命中数等于仓储读取数，并验证可读数与失败数完整覆盖所有投影结果；这些是调用消减证据，不作为墙钟加速声明。
- 投影/同步健康定向 2 文件/3 项、MVP API 聚合 1 项和默认 supplement task 路径 1 项通过；Server lint/build 与 `git diff --check` 通过；沙箱外最终 Server 全量 220 文件/1818 项通过、1 文件/2 项既有跳过、0 失败。实现提交为 `779cb324`。
- 本切片没有写项目仓储、源故事仓储或 `data/provinces/*.md`，没有执行恢复，也没有授予外部证据、真人审核、外部模型、生产流量或成片信用。下一有界切片应先审计 generated health 的项目读取能否复用该 current-state 合同；若需要历史版本、归档系列或不同失效语义则保持独立，不做不安全的强行合并。

M4 第五十五切片完成时：

- generated health 合同审计确认其不是纯 current-state 消费者：普通项目必须保留全版本读取以识别断裂的 `current_version_id/current_story_id`，同时还要聚合系列项目、生成故事文件、软归档排除和 generation activity。因此本切片没有用 projection snapshot 替换原健康扫描，也没有把 production portfolio 强行并入。
- 请求级投影新增受控 `seedCurrentState`：只有项目 ID 已由同一仓储列表确认、meta/snapshot 身份与 current version/story 全部验证为 `readable`、且缓存尚未存在时才能预热；未列出项目、损坏 current-state 或重复预热均拒绝，禁止覆盖。边界继续固定 request-scoped、read-only、无写权限、无跨请求缓存。
- MVP generated health 在完成原有全版本读取后，以同一份已解析 current meta/snapshot 尝试预热；随后 supplement snapshot 与 sync-health 成为第二、第三消费者。新增 seeded/rejected 诊断，聚合回归验证缓存命中数严格等于 `2 × seeded + repository current-state reads`，可读/失败总数严格覆盖 seeded 与仓储回退读取。
- 新等价性测试用两份物理版本证明：启用投影前后的 generated health `summary/items` 完全相同、`versions=2` 证据保留；后续两次读取均命中缓存，current-state 仓储调用为 0。投影/同步相关定向 3 文件/4 项、MVP API 与 generated-health 历史/系列 API 各 1 项通过；Server lint/build 与 `git diff --check` 通过；沙箱外最终 Server 全量 221 文件/1819 项通过、1 文件/2 项既有跳过、0 失败。实现提交为 `5e2f0e6a`。
- 本切片不写项目、源故事或省级 Markdown，不执行恢复，不授予外部证据、真人审核、外部模型、生产流量或成片信用；仓储调用消减仍不是墙钟性能声明。下一有界切片优先建立真实大仓库只读投影诊断快照或定位 production portfolio 剩余独立成本，但必须保留历史版本与系列语义。

M4 第五十六切片完成时：

- 新增 `story-agent-mvp-read-only-projection-diagnostic/v1` 与 `audit:story-agent-mvp-read-only-projection`。`--write` 对真实普通项目、系列项目、故事文件和 `data/provinces` 做内容指纹，执行完整 MVP 只读聚合，再复核运行后指纹；报告绑定共享类型、generated health、projection、supplement、sync-health、MVP service、脚本自身和 npm 入口源码 SHA-256。
- 诊断把确定性门禁与墙钟观测分离：硬门禁验证项目列表仓储调用为 1、后续列表缓存至少 2、`current_state_cache_hits = 2 × seeded + repository_reads`、`readable + failed = seeded + repository_reads`、sync-health 全项目覆盖、机器只读和外部证据信用未授予。墙钟固定标记 `wall_clock_observation_not_sla=true`。
- 真实仓储最终快照：普通项目 2051、系列项目 963；current-state seeded 2051、rejected 0、repository call 0、cache hit 4102、readable 2051、failed 0。项目列表 repository/cache 为 1/2。最终观测总墙钟 24808ms，generated health 3646ms、supplement snapshot 3421ms、production portfolio 15344ms、sync-health 2336ms；production portfolio 仍是最大独立成本，但不得据单次运行声明回归或提速。
- 运行前后四类根内容指纹均为 `55ceef81363b12a8387b289e9e2edb9652e69a3f2b5a9d205d82c9b012066827`，报告 SHA-256 为 `a04c5b112bd3361574de707ec916d10932211046c5d8bc41e8bcd21f65827b46`；最终 `--check` 复核报告自哈希、确定性方程、源码哈希与当前仓储指纹全部通过。
- 诊断定向 1 文件/2 项、Server lint/build 与 `git diff --check` 通过。首次完整 Server 门禁有 1 个无关地点检索用例瞬时返回 401；该路由无鉴权中间件，单项与完整 `api.test.ts` 249/249 均通过，未修改产品代码；第二次完整门禁为 222 文件/1821 项通过、1 文件/2 项既有跳过、0 失败，门禁后诊断 `--check` 仍通过。实现提交为 `39b79f6b`。
- 本切片不写业务仓储、不修改省级 Markdown、不调用外部 provider、不授予人工或生产信用。下一有界切片优先审计 production portfolio 内部普通项目/系列项目扫描的重复成本，在保持历史版本、系列、全量 summary 和失败语义的前提下寻找独立优化点。
...
- 第五十三切片的新 canary-decision 报告和原 prompt-shadow 报告均在沙箱外运行 `tsx --write/--check`，完整 Server/Supertest 也在沙箱外运行；这些证据只证明本地合成 fixture 的确定性绑定和 fail-closed 决策，不代表真实人工签署或生产 canary。
- 第五十四切片的 MVP API 定向与完整 Server/Supertest 在沙箱外运行，以避开当前环境已知监听 `EPERM`；投影只证明请求内仓储调用去重和失败隔离，没有运行大仓库墙钟基准，因此不得转述为端到端延迟已经降低。
...
npm run audit:story-knowledge-prompt-shadow-canary-decision -- --check
...
#### 已入库的剩余开发 backlog（第五十六切片后；15×3 正式快照为 v34）
...
3. 外部证据接入与校验：自动草拟、九类候选导入、受控 multipart 制品上传、项目制品路径/hash/范围验证、HTTPS DNS 钉扎安全抓取与项目缓存、验收/拒绝/撤销状态机、追加式 hash 审计、候选状态 CAS、项目—源故事跨存储崩溃恢复、项目级/全局只读同步健康审计及 MVP 状态 lane/next-actions 均已完成。2051 普通项目 + 963 系列规模的精确故事查找、固定并发、组件耗时诊断、supplement/writeback 单次任务快照与重型磁盘扫描错峰均已完成；真实诊断进一步证明 2051/2051 current-state 由 generated health 预热、0 仓储回退、4102 后续缓存命中，运行前后业务仓储指纹不变。production portfolio 仍需历史版本和系列项目，最终单次观测约 15.3 秒；下一步审计其内部普通/系列扫描重复点，不强行并入 current-state 投影。真实外部证据仍须坚持 reviewer、hash、范围和账本链要求，不得用请求自报、生成正文、机器计划或“待确认”措辞伪造证据。
...
- B4 与 M4 第一至第五十六切片均已完成；第五十六切片实现提交为 `39b79f6b`，交接文档提交与上游状态以最新 `git log` 与 `git status -sb` 为准。v34 正式快照的 pattern 开放目标与弱信号均为 0；RAG 当前工程成熟度评估为代码层 94%、检索层 85%、生成层 94%、综合约 91%。19 案例检索基线、289 条生成准备 shadow、7 案例不可消费 prompt hash/diff 矩阵、受限只读 canary、receipt 绑定验证和 7 路迁移决策报告均已完成。MVP 真实只读诊断已固化 2051 普通项目、963 系列、2051 seeded、4102 cache hit、0 current-state repository call/失败及业务仓储无写指纹；知识 shadow 仍受真实证据约束，production portfolio 继续保持独立历史/系列合同。
