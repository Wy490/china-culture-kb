...
function renderGearsExecutionWorkerEvidenceBundleMarkdown(
  ...
  const lines: string[] = [
    ...
    `- real_gears_callback_base_public: ${bundle.real_gears_callback_base_public}`,
    `- real_gears_acceptance_ready_to_run: ${bundle.real_gears_acceptance_ready_to_run}`,
    `- real_gears_acceptance_blocker: ${bundle.real_gears_acceptance_blocker || 'none'}`,
    `- local_acceptance_counts_as_real_external_callback: ${bundle.local_acceptance_counts_as_real_external_callback}`,
    `- seedance_provider_external_loop_ready: ${bundle.seedance_provider_external_loop_ready}`,
...
export async function getGearsExecutionWorkerEvidenceBundle(): Promise<GearsExecutionWorkerEvidenceBundle> {
  ...
  const bundle: Omit<GearsExecutionWorkerEvidenceBundle, 'markdown'> = {
    ...
    real_gears_callback_base_public: mvpStatus.summary.real_gears_callback_base_public,
    real_gears_acceptance_ready_to_run: mvpStatus.summary.real_gears_acceptance_ready_to_run,
    real_gears_acceptance_blocker: mvpStatus.summary.real_gears_acceptance_blocker,
    local_acceptance_counts_as_real_external_callback: mvpStatus.summary.local_acceptance_counts_as_real_external_callback,
    seedance_provider_external_loop_ready: mvpStatus.summary.seedance_provider_external_loop_ready,
...
export async function getGearsExecutionWorkerEvidenceSignoffReport(
  ...
  const rawVerdictGates = evidenceArray(verdict?.gates)
    .filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === 'object' && !Array.isArray(item));
  const verdictMvpGate = rawVerdictGates.find(item => item.id === 'story_agent_mvp_status_audit');
  const verdictSystemExternalGate = rawVerdictGates.find(item => item.id === 'system_external_callback_batch');
