...
### 5.0 Agent 知识观：知识库不是资料仓库

新的平台原则：Agent 真正需要的不是“更多文件”，而是可检索、可约束、可决策的知识结构。资料仓库只回答“有什么材料”，知识库必须回答“这些材料在生成中应如何使用、不能如何使用、和其他条目是什么关系”。

落到平台架构上：

- Domain Pack 不只是数据解析器，还必须声明条目角色、关系类型、可信度边界、生成用途和风险红线。
- Platform Agent 路由时不能只把命中的文本片段塞进 prompt，而要构造知识包：主条目、支撑条目、时代设定、资产包、母题包、风险提示。
- 子 Agent 生成时必须把知识包转成创作决策：人物身份、行动动机、场景资产、道具归属、时代边界、伏笔/回收、可信度说明。
- GEARS 输出应继承这些结构化边界，保证分镜和视频提示词不会把支撑资料误写为史实。
- 资料补录的结果需要进入结构字段，而不是只追加一段“补充资料说明”。

这条原则适用于所有未来领域：文化、警务、小说改编、商业宣传都要有各自的知识结构和红线，而不是共享一个无差别资料池。
