...
2. 支持导出系列 bible，包含主线骨架、角色弧线、长期线索、连续性账本和分集蓝图。
3. 支持系列项目复制、归档、删除。
4. 支持批量检查所有分集卡片的伏笔/回收闭环。
5. 在单集生成前展示将要带入的“生成上下文预览”。
...
> 仓库 `/Users/wuyu/Desktop/china-culture-kb`，继续完善 AI 漫剧长篇创作。当前已有系列规划页 `/ai-comic-series/new`、系列项目保存、分集卡片编辑、单集分镜生成、连续性账本、系列质量审计、分集蓝图、已生成分镜跳转、计划变更提示、账本重建和下一集推荐生成。请先阅读 `docs/ai-comic-series-longform-handoff.md` 和相关文件，再继续实现下一步：系列 bible 导出、系列项目复制/归档/删除，或单集生成上下文预览。
