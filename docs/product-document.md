...
**一句话描述**：从文化素材到视频脚本的 AI 影视前期生产平台——浏览素材库 → 生成剧情方案 → 输出 GEARS 分段脚本 → 衔接视频生产。
...
当前 GEARS 接入已完成第一阶段 API 拉取交接：

- 前端 GearsActions 组件提供复制 JSON、导出文件、复制单段脚本
- 供稿包支持 Markdown / JSON 复制导出，并可保存人工编辑稿
- "复制 GEARS 拉取配置"会复制 `segments_url` 与 `delivery_url` 的绝对地址
- grears v2 第一阶段通过 API 主动拉取分段数据和供稿包
- 配置 `GEARS_WEBHOOK_URL` 后，故事生成完成会异步发送 `story_ready` 通知
...
grears v2 前端/后端
       │
       │  GET /api/stories/:storyId/gears-delivery
       │  GET /api/stories/:storyId/gears-segments
       │  （优先使用供稿包，保留 segments 做版本兼容）
       ↓
...
1. 在故事详情或生成结果页点击"复制 GEARS 拉取配置"，获得 `delivery_url` 与 `segments_url`
2. grears v2 优先调用 `GET /api/stories/:storyId/gears-delivery`，读取人物资产、场景资产、剧本单元和供稿 Markdown
3. 如需兼容旧链路，再调用 `GET /api/stories/:storyId/gears-segments`
4. 根据 `schema_version` 判断字段兼容性（v2 新增了 video_type / presentation_style / segment_prompt_hint / source_entries）
5. 从 `units[]` 或 `segments[]` 中提取脚本、镜头提示、画面重点和合规提示，驱动分镜生产
6. 若 `validation_notes` 非空，先在知识库平台完成资料补录或人工供稿编辑

**优势**：
- 接入成本最低，只需 HTTP GET 请求
- 知识库平台已提供可复制拉取配置，不再需要手工拼 URL
...
第二阶段（可选启用）
  └── 模式 B：设置 GEARS_WEBHOOK_URL 环境变量并对接 grears v2 webhook
      └── 故事生成完成后自动推送通知
