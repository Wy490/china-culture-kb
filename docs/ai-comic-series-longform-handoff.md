...
2. 给已生成分镜增加“打开故事项目”链接，使用 `generated_episode_story_ids[episode_no]` 跳转到现有故事详情。
3. 当已生成过的分集卡片被编辑时，显示“已生成，修改后建议重新生成”提示。
4. 支持从某集生成后自动建议下一集生成，带入账本状态。
5. 支持“只重建账本”或“从第 N 集重新推演账本”，解决中途编辑前文后的连续性修复。
...
5. 支持导出系列 bible。
6. 支持系列项目复制、归档、删除。
7. 支持批量检查所有分集卡片的伏笔/回收闭环。

## 新对话接手提示

建议新对话从这里继续：

> 仓库 `/Users/wuyu/Desktop/china-culture-kb`，继续完善 AI 漫剧长篇创作。当前已有系列规划页 `/ai-comic-series/new`、系列项目保存、分集卡片编辑、单集分镜生成、连续性账本，并且单集生成已经会读取保存项目里的 ledger。请先阅读 `docs/ai-comic-series-longform-handoff.md` 和相关文件，再继续实现下一步：把已生成分镜链接到故事项目详情，并在已生成分集被编辑时提示需要重新生成。
