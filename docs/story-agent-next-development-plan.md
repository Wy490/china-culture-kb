...
| AI 漫剧系列生产链 | 约 70% | 系列规划、生产账本、回片、剪辑包、缩略图、精修计划、SRT 字幕包、字幕 worker、音频计划、混音 dry-run、片头片尾计划/render dry-run、final delivery dry-run、final manifest、外部剪辑平台包和生产总览 dashboard 首版已有；下一步是真实混音、真实片头片尾渲染、真实最终装配和审片返修。 |
...
- AI 漫剧 Seedance 片头片尾 / final delivery dry-run 首版：
  ...
  - 服务测试覆盖 title card plan、render dry-run、strict 缺依赖、final delivery dry-run 和 manifest 写盘；API 测试覆盖 title card/final 请求校验和缺失项目响应。
