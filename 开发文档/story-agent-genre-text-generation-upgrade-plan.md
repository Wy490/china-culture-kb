...
1. 系列规划增加“主线剧情骨架”；（已完成：`AiComicSeriesPlan.series_spine` 输出阶段级主线节点，前端展示）
2. 每集卡片增加：（已完成：规划生成、前端展示与编辑均支持以下字段）
   - 本集开场钩子；
   - 中段转折；
   - 结尾钩子类型；
   - 角色状态变化；
   - 线索开合动作；
3. 单集生成前构造 `AiComicEpisodeBlueprint`；（已完成：蓝图进入生成提示，并随 `StoryGenerateResult.ai_comic_episode_blueprint` 返回）
4. 单集生成后校验：（已完成：单集质量报告会参考承接、阶段推进、角色状态、线索动作与结尾钩子）
   - 是否回应上一集结尾；
   - 是否推进本阶段目标；
   - 是否更新角色状态；
   - 是否打开/回收线索；
   - 是否为下一集留下明确承接；
5. 分集卡片被编辑后标记：（已完成：系列级审计会识别已生成分集的规划变更，前端提示需重新生成并可从最早变更集数重建账本）
...
3. 故事详情页展示质量报告；（已完成：故事详情复用 `StoryResult` 展示质量摘要、缺失要素、节拍定位与场景高亮）
4. 项目详情页支持按质量问题筛选场景；（已完成：项目详情页展示当前版本质量摘要，可仅查看质量报告关联场景）
5. 局部重写后重新运行类型片校验；（已完成：局部重写服务会重新生成质量报告，项目详情页在成功提示中显示复核结果）
...
2. StoryStudio 增加高级设置：
   ...
   - 是否开启自动修复；（已完成：StoryStudio 请求接入 `auto_repair`，AI 漫剧单集生成接入 `auto_repair_episode`）
3. StoryResult 展示：（已完成：结果页展示质量摘要、类型匹配度、缺失要素、节拍定位、建议调整与场景高亮）
   - 类型匹配度；
   - 场景功能；
   - 类型专属字段；
   - 质量问题；
4. AI 漫剧系列页展示：（已完成：系列页展示账本、系列质量审计、分集状态、计划变更提示、账本重建与推荐生成入口）
   - 连续性账本；
   - 每集质量状态；
   - 已生成但计划已变化提示；
