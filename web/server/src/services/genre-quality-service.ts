...
function hasSemanticSignalEvidence(text: string, signal: string): boolean {
  ...
  const checks: Array<[RegExp, RegExp[]]> = [
    ...
    [/人物不是年表|不得写成年表式介绍/, [/(少年周敦颐|周敦颐).*(背起|停下脚步|蹲下|挽起|踩进|写下)/, /(屈原).*(站在风里|走向汨罗江|整理衣冠|怀石)/]],
    [/史实边界明确|再现边界清楚/, [/影视化创作/, /确证/, /不是《爱莲说》/, /不把.{0,20}写成/, /仍要说清/, /只作.{0,8}伏笔/, /据《史记》/, /传统叙述/, /有限再现/, /不把再现画面当作原始记录/, /事实梳理/]],
    [/制度压力可见|时代压力可见/, [/郢都失守/, /楚国败局/, /国都失陷/, /流放无归/, /制度压力/, /上官/, /催签/, /权势/]],
